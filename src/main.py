...
def main():
    ...
    intrinsic_m = np.array([[param.img_fx, 0, param.img_cx],
                            [0, param.img_fy, param.img_cy], [0, 0, 1]])
    ...
    for image_name in image_names:
        ...
        if image_id > begin_id:
            ...
            feature2d[:,
                      0] = feature2d[:, 0] * cam.fx / vo.feature3d[:,
                                                                   2] + cam.cx
            feature2d[:,
                      1] = feature2d[:, 1] * cam.fx / vo.feature3d[:,
                                                                   2] + cam.cy
            ...
            if vo.feature3d.shape[0] > param.minimum_feature_for_scale:
                pitch = scale_estimator.initial_estimation(
                    vo.motion_t.reshape(-1))
                pitchs.append(pitch)
                scale, std = scale_estimator.scale_calculation(
                    vo.feature3d, feature2d, img_bgr)
                if real_scale is not None:
                    print('predict,real,std', scale, real_scale[image_id - 1],
                          std)
                ...
                if (False):  #  and abs(real_scale[image_id-1]-scale)>0.3):
                    scale_estimator.check_full_distribution(
                        vo.feature3d.copy(), feature2d.copy(),
                        real_scale[image_id - 1], img_bgr)

...
class VisualOdometry:
    def __init__(self, cam, annotations=None):
        ...
        self.detector = dt.FeatureDetector()
    ...
    def processFirstFrame(self, cond_mask=None):
        self.px_ref = self.detector.detect(self.new_frame, cond_mask)
        # self.px_ref = np.array([x.pt for x in self.px_ref], dtype=np.float32)
        self.frame_stage = STAGE_SECOND_FRAME
        # return True

    def processSecondFrame(self, cond_mask=None):
        self.px_ref, self.px_cur = featureTracking(
            self.last_frame, self.new_frame, self.px_ref
        )
        if self.px_cur.shape[0] < 10:
            self.px_cur = self.detector.detect(self.new_frame, cond_mask)
        ...
        _, self.cur_R, self.cur_t, mask, points_3d = cv2.recoverPose(
            E,
            self.px_cur,
            self.px_ref,
            cameraMatrix=self.camera_matrix,
    ...
    def processFrame(self, frame_id, cond_mask=None):
        self.px_ref, self.px_cur = featureTracking(
            self.last_frame, self.new_frame, self.px_ref
        )
        # E, mask = cv2.findEssentialMat(
    ...
    def visualize(self, img):
        point = (
            int(img.shape[1] / 2 + self.cur_t[0, 0]),
            int(img.shape[0] / 2 - self.cur_t[2, 0]),
        )
        cv2.circle(img, point, 1, (0, 255, 0), -1)

    def update(self, img, frame_id, cond_mask=None):
        assert (
            img.ndim == 2
        ...
        if self.frame_stage == STAGE_DEFAULT_FRAME:
            flag = self.processFrame(frame_id, cond_mask)
        elif self.frame_stage == STAGE_SECOND_FRAME:
            flag = self.processSecondFrame(cond_mask)
        elif self.frame_stage == STAGE_FIRST_FRAME:
            flag = self.processFirstFrame(cond_mask)

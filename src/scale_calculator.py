...
class ScaleEstimator:
    ...
    def check_distance(self, feature3d):
        flag_x = (
            feature3d[:, 2] * (feature3d[:, 2] + 1)
            - np.abs(self.focus * feature3d[:, 0])
        ) < 0
        flag_y = (
            feature3d[:, 2] * (feature3d[:, 2] + 1)
            - np.abs(self.focus * feature3d[:, 1])
        ) < 0
        flag = flag_x | flag_y
        ...
        for valid_triangle_id in valid_triangle_ids:
            ...
            for mb_id in mb_ids:
                ...
                potential_matrix = np.array(
                    [(1 - pa) * (1 - pc), (1 - pa) * pc, pa * (1 - pc), pa * pc]
                )
                pa = (
                    observation_matrix[2:4, cr + 1]
                    @ potential_matrix[2:4]
                    / (observation_matrix[:, cr + 1] @ potential_matrix)
                )
                prab.append(pa)
                # print(pa)
    ...
    def remove_single(self, feature3d, dis, bins):
        ...
        if np.sum(bins_single) > 0:
            ...
            unvalid_flag = (feature3d[:, 1] >= bin_single - 0.1) & (
                feature3d[:, 1] <= bin_single
            )
    ...
    def feature_remap(self, feature3d):
        y = feature3d[:, 1] * np.cos(self.camera_pitch) - feature3d[:, 2] * np.sin(
            self.camera_pitch
        )
        z = feature3d[:, 1] * np.sin(self.camera_pitch) + feature3d[:, 2] * np.cos(
            self.camera_pitch
        )
        feature3d[:, 1] = y
        feature3d[:, 2] = z
    ...
    def mode_analysis(self):
        dis, bins = np.histogram(
            self.flat_feature[:, 1], bins=np.array(range(0, 100)) * 0.1, density=True
        )
    ...
    def plot_distribution(self, label, img, scale=1):
        ...
        if self.all_feature is not None and len(self.all_feature) > 0:
            ...
            dis, bins = np.histogram(
                all_feature[:, 1], bins=np.array(range(0, 50)) * 0.1
            )
        ...
        if len(self.correct_distance_feature) > 0:
            ...
            dis, bins = np.histogram(
                correct_feature[:, 1], bins=np.array(range(0, 50)) * 0.1
            )
        ...
        if self.flat_feature is not None and len(self.flat_feature) > 0:
            ...
            dis, bins = np.histogram(
                flat_feature[:, 1], bins=np.array(range(0, 50)) * 0.1, density=True
            )
        ...
        ax3 = plt.subplot(222)
        ax3.plot(
            self.all_feature[:, 2], -self.all_feature[:, 1], ".r", label="All Features"
        )
        ax3.plot(
            self.correct_distance_feature[:, 2],
            -self.correct_distance_feature[:, 1],
            ".g",
            label="Depth-correct Features",
        )
        ax3.set_xlabel("z")
        ax3.set_ylabel("-y")

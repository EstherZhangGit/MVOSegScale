...
def ses_mean(se_datas):
    ...
    se_datas = np.transpose(
        se_datas.reshape(
            se_datas.shape[0], se_datas.shape[1], se_datas.shape[2] * se_datas.shape[3]
        ),
        (0, 2, 1),
    )
...
def sos2quats(so_datas, mean_std=[[1], [1]]):
    ...
    so_datas = so_datas.reshape(
        so_datas.shape[0], so_datas.shape[1], so_datas.shape[2] * so_datas.shape[3]
    )
